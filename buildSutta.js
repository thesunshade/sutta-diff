export function buildSutta(slug, suttaArea) {
  console.log(slug);
  let translator = "";
  ...
  Promise.all([contentResponse, suttaplex])
    .then(responses => {
      ...
      keys_order.forEach(segment => {
        ...
        html += `${openHtml}${translation_text[segment]}${closeHtml}<br>\n`;
      });
    ...
    .catch(error => {
      return `<p>Sorry, "${decodeURIComponent(slug)}" is not a valid sutta citation.
      <br><br>
      Note: <br>
